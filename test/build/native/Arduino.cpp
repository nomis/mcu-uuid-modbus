...
void ModbusDevice::respond() {
	if (rx_[0] == 0x7B
		...
		tx_.insert(tx_.end(), { 0x7B, 0x04, 0x04, 0x12, 0x34, 0x56, 0x78,
			0x90, 0xAB, 0xCD, 0xEF, 0xEA, 0xC2 });
		delay(50);
	} else if (rx_[0] == 0x7B
			&& rx_[1] == 0x04
			&& rx_[2] == 0x00
			...
			&& rx_[7] == 0xB0) {
		rx_.erase(rx_.begin(), rx_.begin() + 8);
		tx_.insert(tx_.end(), { 0x7B, 0x04, 0x04, 0x23, 0x45, 0x67, 0x89,
			0x0A, 0xBC, 0xDE, 0xF1, 0x23, 0x0D });
		delay(50);
	} else if (rx_[0] == 0x7B
			&& rx_[1] == 0x04
			&& rx_[2] == 0x00
			...
			&& rx_[7] == 0xB3) {
		rx_.erase(rx_.begin(), rx_.begin() + 8);
		tx_.insert(tx_.end(), { 0x7B, 0x04, 0x04, 0x34, 0x56, 0x78, 0x90,
			0xAB, 0xCD, 0xEF, 0x12, 0x79, 0x75 });
		delay(50);
	} else if (rx_[0] == 0x7B
			&& rx_[1] == 0x04
			&& rx_[2] == 0x00
			...
			&& rx_[7] == 0x72) {
		rx_.erase(rx_.begin(), rx_.begin() + 8);
		tx_.insert(tx_.end(), { 0x7B, 0x04, 0x04, 0x45, 0x67, 0x89, 0x0A,
		...
		delay(50);
			...
			&& rx_[7] == 0xB4) {
		rx_.erase(rx_.begin(), rx_.begin() + 8);
		tx_.insert(tx_.end(), { 0x7B, 0x84, 0x02, 0xE3, 0x18 });
		delay(50);
	} else if (rx_[0] == 0x7B
			&& rx_[1] == 0x04
			&& rx_[2] == 0x00
			...
			&& rx_[7] == 0x75) {
		rx_.erase(rx_.begin(), rx_.begin() + 8);
		tx_.insert(tx_.end(), { 0x7B, 0x04, 0x03, 0x56, 0x78, 0x9A, 0xBC,
			0xDE, 0xF0, 0x34, 0x15 }); // Unexpected number of registers
		delay(50);
	} else if (rx_[0] == 0x7B
			&& rx_[1] == 0x04
			&& rx_[2] == 0x00
			...
			&& rx_[7] == 0x76) {
		rx_.erase(rx_.begin(), rx_.begin() + 8);
		tx_.insert(tx_.end(), { 0x7B, 0x04, 0x04, 0x67, 0x89, 0xAB, 0xCD,
			0xEF, 0x01, 0x23, 0x45, 0x67, 0x89, 0x76, 0xC2 }); // Length mismatch
		delay(50);
	}
}
